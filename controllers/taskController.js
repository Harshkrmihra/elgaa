...
exports.updateTask = [
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      req.user.id !== task.coach.toString() &&
      req.user.id !== task.customer.toString()
    ) {
      // Authorization check
      return res
        .status(403)
...
exports.deleteTask = [
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.id !== task.coach.toString()) {
      // Authorization check
      return res
        .status(403)
...
exports.getTasksByCustomer = [
  auth, // Authentication middleware
  asyncHandler(async (req, res) => {
    if (req.user.id !== req.params.customerId && req.user.type !== "Coach") {
      // Authorization check
