    ...
    if (type === "Admin" || type === "Coach") {
      if (!req.user) {
        return res.status(401).json({
          msg: "Authorization required to create an Admin or Coach user",
        });
      }
      ...
      if (authenticatedUser.type !== "Admin") {
        return res
          .status(403)
          .json({ msg: "Only admins can create new admin or coach accounts" });
      }
    } else if (type !== "Customer") {
      return res.status(400).json({
        msg: "Invalid user type. User type can be Admin, Coach or Customer",
      });
    }
