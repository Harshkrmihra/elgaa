...
const api = axios.create({
  baseURL: "http://localhost:3002/api", // Update the base URL to match your backend API address
});
...
export const getCoachByUsername = async (username) => {
  try {
    const response = await axios.get(`http://localhost:3002/@${username}`);
...
export const getGameByFriendlyUrl = async (friendlyUrl) => {
  try {
    const response = await axios.get(`http://localhost:3002/${friendlyUrl}`);
    return response.data;
  } catch (error) {
    throw error;
  }
